...
describe Slugalicious do
  ...
  context "slug generation" do
    ...
    it "should generate from a proc as well as a symbol" do
      User.send :slugged, lambda { |person| "#{person.first_name} #{person.last_name}" }
    ...
    it "should raise an error if all generators return nil" do
      ...
      lambda { Factory(:user, :gender => nil, :birthdate => nil) }.should raise_error
    end
    ...
    it "should accept a proc for a scope" do
      User.send :slugged, :first_name, :last_name, :scope => lambda { |object| object.callsign[0] }
    ...
    it "should raise an error if no unique slugs are available" do
      ...
      User.send :slugged, lambda { |object| 'f' }, :blacklist => 'f'
      lambda { Factory(:user, :first_name => 'Foo', :last_name => 'Bar') }.should raise_error
  ...
  describe "#find_from_slug" do
    ...
    it "should raise ActiveRecord::RecordNotFound if the slug does not exist" do
      lambda { User.find_from_slug('nonexist') }.should raise_error(ActiveRecord::RecordNotFound)
    end
    ...
    it "should raise ActiveRecord::RecordNotFound if the slug does not exist in scope" do
      ...
      lambda { User.find_from_slug('fn2', 'ln1') }.should raise_error(ActiveRecord::RecordNotFound)
    end
